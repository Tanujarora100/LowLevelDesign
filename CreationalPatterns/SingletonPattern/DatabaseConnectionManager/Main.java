...
public class Main {
    ...
    public static void main(String[] args) throws IOException, ClassNotFoundException,NotSerializableException, InvocationTargetException, InstantiationException, IllegalAccessException {
        //exampleSerialization();
        exampleSerialization();
    }

    private static void exampleSerialization() throws IOException, ClassNotFoundException {
        LazySingletonDBManager lazySingleton = LazySingletonDBManager.getInstance();
        ...
        LazySingletonDBManager deserializedLazy = (LazySingletonDBManager) objectInputStream.readObject();
    ...
    private static void exampleReflection() throws InvocationTargetException, InstantiationException, IllegalAccessException {
        Constructor[] constructors = LazySingletonDBManager.class.getDeclaredConstructors();
        //Knowing only one constructor taking it using index
        Constructor constructor = constructors[0];
        constructor.setAccessible(true);
        LazySingletonDBManager lazySingleton = (LazySingletonDBManager) constructor.newInstance();
        LazySingletonDBManager instance = LazySingletonDBManager.getInstance();
